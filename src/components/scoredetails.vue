    ...
    <div class="scoredetails-container">
      ...
      <div class="scoredetails-content">
        <div class="scoredetails-list">
          ...
          <div class="scoredetails-item" v-for="item in Arr" :key="item._id">
            <div class="scoredetails-icon1">
              <div class="typename">{{item.scoreName}}</div>
              <div class="timefor">{{item.dateTime}}</div>
            </div>
...
<script>
    export default {
      ...
      methods:{
        getScore(){
          this.$axios.get("getScore").then(res=>{
            console.log(res);
            this.Arr=res
            for (var i=0;i<this.Arr.length;i++){
              ...
              this.Details[i].dateTime = Y+M+D+h+m+s
            }
          })
        }
      },
...
<style scoped>
  .scoredetails-content{margin-top: 50px}
 .scoredetails-item{
   background: #fff;
   overflow-y: auto;
   left: 0;right: 0;
